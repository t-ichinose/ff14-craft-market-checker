...
// インフライトリクエスト重複排除用キャッシュマップ
const inFlightListings = new Map<string, Promise<FetchListingsResult>>();
const inFlightTriads = new Map<string, Promise<TriadListingsResult>>();
const inFlightHistories = new Map<string, Promise<FetchHistoryResult>>();

/**
 * タイムアウトと外部 AbortSignal を合成したシグナルを生成
 */
...
export async function fetchMarketListings(
  ...
  const { isHq, limit = 50, signal, forceLive, timeoutMs = DEFAULT_TIMEOUT_MS } = options;
  ...
  // インフライト重複排除キー
  const inFlightKey = `${scope}_${itemId}_${isHq !== undefined ? isHq : 'all'}_${limit}`;
  const existingPromise = inFlightListings.get(inFlightKey);
  if (existingPromise) {
    return existingPromise;
  }

  const execPromise = (async () => {
    const { signal: combinedSignal, cleanup } = createTimeoutSignal(timeoutMs, signal);
    try {
      ...
      if (!res.ok) {
        if (res.status === 429) {
          throw new Error('Universalis API rate limit exceeded (HTTP 429)');
        }
      ...
      cleanup();
      inFlightListings.delete(inFlightKey);
    }
  })();

  inFlightListings.set(inFlightKey, execPromise);
  return execPromise;
}
...
export async function fetchMarketHistory(
  ...
  const { isHq, limit = 50, signal, timeoutMs = DEFAULT_TIMEOUT_MS } = options;

  const inFlightKey = `history_${scope}_${itemId}_${isHq !== undefined ? isHq : 'all'}_${limit}`;
  const existingPromise = inFlightHistories.get(inFlightKey);
  if (existingPromise) {
    return existingPromise;
  }
  ...
  const execPromise = (async () => {
    ...
    try {
      ...
      cleanup();
      inFlightHistories.delete(inFlightKey);
    }
  })();

  inFlightHistories.set(inFlightKey, execPromise);
  return execPromise;
}
...
export async function fetchCheapestTriad(
  ...
  // インフライト重複排除キー
  const triadKey = `triad_${world}_${dc}_${itemId}_${isHq !== undefined ? isHq : 'all'}_${forceLive ? 'live' : 'cached'}`;
  const existingPromise = inFlightTriads.get(triadKey);
  if (existingPromise) {
    return existingPromise;
  }
  ...
  const execPromise = (async () => {
    ...
    try {
      ...
      inFlightTriads.delete(triadKey);
    }
  })();

  inFlightTriads.set(triadKey, execPromise);
  return execPromise;
}
