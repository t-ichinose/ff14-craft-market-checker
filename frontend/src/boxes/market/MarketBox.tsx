...
export const MarketBox: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const isHq = Boolean(selectedItem.hq);
    ...
    const dcWorldSet = new Set(JAPAN_DCS[activeDc] || []);
    const worldEntries = Object.entries(allData);

    for (let w = 0; w < worldEntries.length; w++) {
      const [wName, wItems] = worldEntries[w];
      for (let i = 0; i < wItems.length; i++) {
        const it = wItems[i];
        if (it.item_id === iid && Boolean(it.hq) === isHq) {
          const p = it.min_price;
          if (p > 0) {
            if (dcWorldSet.has(wName)) {
              if (p < dcMinPrice) {
                dcMinPrice = p;
                dcMinWorld = wName;
              }
