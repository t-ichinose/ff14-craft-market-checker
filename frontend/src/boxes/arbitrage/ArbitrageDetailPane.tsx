        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.2rem' }}>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: '0.65rem', color: '#94a3b8' }}>
              想定日当利益 ({selectedOp.stockQuantity < (selectedOp.velocity || 0) ? `仕入在庫 ${selectedOp.stockQuantity}個上限` : `自鯖日販 ${(selectedOp.velocity || 0).toFixed(1)}個上限`}):
            </div>
            <div style={{ fontSize: '1.15rem', fontWeight: 800, color: selectedOp.dailyProfit > 0 ? '#4ade80' : '#94a3b8', fontFamily: 'Outfit, sans-serif' }}>
              {selectedOp.dailyProfit > 0 ? `+${selectedOp.dailyProfit.toLocaleString()}G/日` : `${selectedOp.dailyProfit.toLocaleString()}G/日`}{' '}
              <span style={{ fontSize: '0.72rem', color: selectedOp.roiPercent > 0 ? '#ffb703' : '#64748b', fontWeight: 'normal' }}>
                (単価 +{selectedOp.unitProfit.toLocaleString()}G / {selectedOp.roiPercent > 0 ? `+${selectedOp.roiPercent}%` : `${selectedOp.roiPercent}%`})
              </span>
            </div>
          </div>
          <a
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.85rem', flex: 1, minHeight: 0, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden', background: 'rgba(15, 23, 42, 0.7)', border: '1px solid rgba(74, 222, 128, 0.35)', borderRadius: '12px', padding: '0.75rem', boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.5rem', paddingBottom: '0.4rem', borderBottom: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
            ...
            <span style={{ fontSize: '0.72rem', color: '#94a3b8' }}>
              目標仕入値: <strong style={{ color: '#c084fc', fontSize: '0.85rem', fontFamily: 'Outfit, sans-serif' }}>
                {selectedOp.maxBuyPrice ? `${selectedOp.maxBuyPrice.toLocaleString()}G以下` : (selectedOp.buyPrice > 0 ? `${selectedOp.buyPrice.toLocaleString()}G` : '-')}
              </strong>
            </span>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px', marginBottom: '0.65rem', flexShrink: 0 }}>
            <div style={{ background: 'rgba(0,0,0,0.35)', border: '1px solid rgba(255,255,255,0.08)', borderLeft: '4px solid #4ade80', borderRadius: '8px', padding: '5px 7px' }}>
              <div style={{ fontSize: '0.62rem', color: '#94a3b8', marginBottom: '2px' }}>最安仕入値</div>
              <div style={{ fontSize: '0.82rem', fontWeight: 800, color: '#4ade80', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'Outfit, sans-serif' }}>
                {selectedOp.buyPrice > 0 ? `${selectedOp.buyPrice.toLocaleString()}G` : '-'}
              </div>
            </div>
            <div style={{ background: 'rgba(0,0,0,0.35)', border: '1px solid rgba(255,255,255,0.08)', borderLeft: '4px solid #c084fc', borderRadius: '8px', padding: '5px 7px' }}>
              <div style={{ fontSize: '0.62rem', color: '#94a3b8', marginBottom: '2px' }}>目標仕入値 (上限)</div>
              <div style={{ fontSize: '0.82rem', fontWeight: 800, color: '#c084fc', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'Outfit, sans-serif' }}>
                {selectedOp.maxBuyPrice ? `${selectedOp.maxBuyPrice.toLocaleString()}G以下` : `${selectedOp.buyPrice.toLocaleString()}G`}
              </div>
            </div>
          ...
          <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.4rem', flexShrink: 0 }}>
              <div style={{ display: 'flex', gap: '3px', background: 'rgba(0,0,0,0.3)', padding: '2px', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.06)' }}>
                <button
                  ...
                  <tbody>
                    {displayedSourceListings.length > 0 ? (
                      displayedSourceListings.map((l, i) => {
                        const targetLimit = selectedOp.maxBuyPrice || selectedOp.buyPrice;
                        const isGoodBuy = targetLimit > 0 ? l.pricePerUnit <= targetLimit : l.pricePerUnit <= selectedOp.buyPrice;
                        const diffPct = targetLimit > 0
                          ? Math.round(((l.pricePerUnit - targetLimit) / targetLimit) * 100)
                          : 0;
                        const diffPctStr = diffPct > 0 ? `+${diffPct}%` : `${diffPct}%`;
                        const isLowest = selectedOp.buyPrice > 0 && l.pricePerUnit <= selectedOp.buyPrice;
                        ...
                        return (
                          <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', background: rowBg }}>
                            ...
                            <td style={{ padding: '4px 6px', textAlign: 'center' }}>
                              {isLowest ? (
                                <span style={{ background: 'rgba(74, 222, 128, 0.22)', color: '#4ade80', border: '1px solid rgba(74, 222, 128, 0.5)', padding: '1px 6px', borderRadius: '4px', fontSize: '0.64rem', fontWeight: 700, whiteSpace: 'nowrap', display: 'inline-flex', alignItems: 'center', gap: '3px' }}>
                                  <i className="fa-solid fa-crown" style={{ fontSize: '0.50rem', color: '#ffb703' }}></i> 最安 ({diffPctStr})
                                </span>
                              ) : isGoodBuy ? (
                                <span style={{ background: 'rgba(56, 189, 248, 0.18)', color: '#38bdf8', border: '1px solid rgba(56, 189, 248, 0.4)', padding: '1px 6px', borderRadius: '4px', fontSize: '0.64rem', fontWeight: 700, whiteSpace: 'nowrap', display: 'inline-flex', alignItems: 'center', gap: '3px' }}>
                                  <i className="fa-solid fa-circle-check" style={{ fontSize: '0.50rem' }}></i> 仕入可 ({diffPctStr})
                                </span>
                              ) : (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden', background: 'rgba(15, 23, 42, 0.7)', border: '1px solid rgba(0, 210, 255, 0.35)', borderRadius: '12px', padding: '0.75rem', boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px', marginBottom: '0.65rem', flexShrink: 0 }}>
            ...
            <div style={{ background: 'rgba(0,0,0,0.35)', border: '1px solid rgba(255,255,255,0.08)', borderLeft: '4px solid #c084fc', borderRadius: '8px', padding: '5px 7px' }}>
              <div style={{ fontSize: '0.62rem', color: '#94a3b8', marginBottom: '2px' }}>目標販売値 (トレンド連動)</div>
              <div style={{ fontSize: '0.82rem', fontWeight: 800, color: '#c084fc', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'Outfit, sans-serif' }}>
