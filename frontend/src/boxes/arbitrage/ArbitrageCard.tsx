    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px', marginBottom: '6px' }}>
        ...
        <div style={{ background: 'rgba(74, 222, 128, 0.05)', border: '1px solid rgba(74, 222, 128, 0.25)', borderLeft: '3px solid #4ade80', borderRadius: '7px', padding: '5px 7px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '3px' }}>
            ...
            <strong style={{ color: '#c084fc', fontSize: '0.78rem', fontFamily: 'Outfit, sans-serif' }}>
              {op.maxBuyPrice ? `${op.maxBuyPrice.toLocaleString()}G以下` : (op.buyPrice > 0 ? `${op.buyPrice.toLocaleString()}G` : '-')}
            </strong>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: '0.60rem', color: '#64748b' }}>
              最安: <strong style={{ color: op.buyPrice > 0 ? '#4ade80' : '#64748b', fontFamily: 'Outfit, sans-serif' }}>{op.buyPrice > 0 ? `${op.buyPrice.toLocaleString()}G` : '-'}</strong>
            </span>
        ...
        <div style={{ background: 'rgba(56, 189, 248, 0.05)', border: '1px solid rgba(56, 189, 248, 0.25)', borderLeft: '3px solid #38bdf8', borderRadius: '7px', padding: '5px 7px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '3px' }}>
            ...
            <strong style={{ color: op.sellPrice > 0 ? '#38bdf8' : '#64748b', fontSize: '0.78rem', fontFamily: 'Outfit, sans-serif' }}>
              {op.sellPrice > 0 ? `${op.sellPrice.toLocaleString()}G` : '-'}
            </strong>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      <div style={{ background: 'rgba(0, 0, 0, 0.35)', border: '1px solid rgba(255, 255, 255, 0.06)', borderRadius: '7px', padding: '5px 8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          <i className="fa-solid fa-coins" style={{ color: op.unitProfit > 0 ? '#ffb703' : '#64748b', fontSize: '0.72rem' }}></i>
          <span style={{ color: '#94a3b8', fontSize: '0.65rem' }}>単価利益:</span>
          <strong style={{ color: op.unitProfit > 0 ? '#ffb703' : '#94a3b8', fontSize: '0.78rem', fontFamily: 'Outfit, sans-serif' }}>
            {op.unitProfit > 0 ? `+${op.unitProfit.toLocaleString()}G` : `${op.unitProfit.toLocaleString()}G`}
          </strong>
          <span style={{ color: op.roiPercent > 0 ? '#fbbf24' : '#64748b', fontSize: '0.64rem', fontFamily: 'Outfit, sans-serif' }}>
            ({op.roiPercent > 0 ? `+${op.roiPercent}%` : `${op.roiPercent}%`})
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          <i className="fa-solid fa-calendar-day" style={{ color: op.dailyProfit > 0 ? '#4ade80' : '#64748b', fontSize: '0.72rem' }}></i>
          <span style={{ color: '#94a3b8', fontSize: '0.65rem' }}>日当利益:</span>
          <strong style={{ color: op.dailyProfit > 0 ? '#4ade80' : '#94a3b8', fontSize: '0.80rem', fontFamily: 'Outfit, sans-serif' }}>
            {op.dailyProfit > 0 ? `+${op.dailyProfit.toLocaleString()}G` : `${op.dailyProfit.toLocaleString()}G`}
            <span style={{ fontSize: '0.58rem', color: '#94a3b8', fontWeight: 'normal', marginLeft: '3px' }}>
              ({op.stockQuantity < (op.velocity || 0) ? `在庫${op.stockQuantity}個` : `日販${(op.velocity || 0).toFixed(1)}個`})
            </span>
          </strong>
        </div>
      </div>
