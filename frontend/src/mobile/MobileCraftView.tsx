...
type SortOption = 'dailyProfit' | 'profit' | 'profitRate' | 'velocity';
...
export const MobileCraftView: React.FC<MobileCraftViewProps> = React.memo(({
  ...
  }, [marketData, sourcingScope, salesWorld, targetDc, listingsMap]);
  ...
  }, [recipesMap, marketData, salesWorld, materialPriceMap, allDcPriceMap, listingsMap]);

  // フィルタ・ソート適用
  const filteredItems = useMemo(() => {
    const q = searchQuery.toLowerCase().trim();
    const isFilterCategory = selectedCategories.length > 0 && availableCategories.length > 0 && selectedCategories.length < availableCategories.length;
    ...
    if (q) {
      ...
      matchedActive.sort((a, b) => {
        switch (sortBy) {
          ...
          case 'velocity':
            return (b.daily_sales_qty || 0) - (a.daily_sales_qty || 0);
          default:
            return b.daily_profit - a.daily_profit;
        }
      });
    ...
    return list;
  }, [allEvaluatedCraftItems, selectedCategories, availableCategories, searchQuery, minVelocity, sortBy, recipesMap, marketData, materialPriceMap, allDcPriceMap]);
  ...
  }, [isActive, selectedItem, recipesMap, marketData, materialPriceMap, sourcingScope, salesWorld, targetDc, materialHqPriceMap, qualityMap, selfSufficientMap]);

  // HQ/NQ切替後の実効製作原価
  const activeCraftCost = useMemo(() => {
    if (!resolvedTree || resolvedTree.length === 0 || !selectedItem) return selectedItem?.craft_cost || 0;
    const totalBatchCost = resolvedTree
      .filter((t) => t.isActive)
  ...
  }, [resolvedTree, selectedItem]);

  // 動的選択アイテム (原価・利益・利益率・日商をリアルタイム同期)
  ...
  const handleSetAllHq = useCallback(() => {
    ...
    function collect(nodes: RecipeTreeItem[]) {
      for (const n of nodes) {
        if (n.hasHqOption) newMap[n.id] = 'hq';
        if (n.subs && n.subs.length > 0) collect(n.subs);
      }
    }
  ...
  const handleSetAllSelfSufficient = useCallback(() => {
    ...
    function collect(nodes: RecipeTreeItem[]) {
      for (const n of nodes) {
        if (!isCrystalItem(n.name)) newMap[n.id] = true;
        if (n.subs && n.subs.length > 0) collect(n.subs);
      }
    }
    collect(resolvedTree);
  ...
  const { intermediateCraftSteps, finalAssemblyMaterials } = useMemo(() => {
        ...
        if (node.method === 'craft' && node.subs && node.subs.length > 0) {
          ...
          if (existing) {
            ...
            existing.materials = node.subs.map((s) => ({
              ...
              world: s.world,
              quality: s.quality,
              hasHqOption: s.hasHqOption,
              isSelfSufficient: s.isSelfSufficient,
            }));
    ...
    const assembly: AssemblyMaterial[] = resolvedTree.map((node) => ({
      ...
      isSelfSufficient: node.isSelfSufficient,
    }));
  ...
  const totalSellPrice = useMemo(() => {
    const item = activeSelectedItem || selectedItem;
    return item ? item.sell_price * craftCount : 0;
  }, [activeSelectedItem, selectedItem, craftCount]);

  const totalCraftProfit = useMemo(() => {
    const item = activeSelectedItem || selectedItem;
    if (!item) return 0;
    const unitProfit = item.profit !== undefined
      ? item.profit
      : item.sell_price - item.craft_cost;
    ...
    <div className="shrink-0 flex flex-col gap-1.5">
      ...
      {isMenuOpen && (
        <div className="flex flex-col gap-1.5 bg-slate-900/95 p-2 rounded-xl border border-white/10 shadow-xl transition-all">
          ...
          {/* Row 3: ソートタブ（日当利益 / 単価利益 / 利益率 / 日販数） ＆ 仕入れスコープ */}
          <div className="flex items-center gap-1.5">
            {/* ソートタブ（4項目） */}
            <div className="flex flex-1 bg-black/40 p-0.5 rounded-lg border border-white/10 text-xs gap-1 items-center">
              <button
                ...
                className={`flex-1 py-1.5 text-center rounded-md font-bold transition-all text-[0.68rem] whitespace-nowrap ${
                  sortBy === 'dailyProfit'
                  ...
                  >
                    ...
                    <div className="grid grid-cols-4 gap-1 bg-black/40 p-1.5 rounded-lg border border-white/5 text-center">
                      <div className="flex flex-col">
                        <span className="text-[0.58rem] text-slate-400">日当利益</span>
                        <span
                      ...
                      <div className="flex flex-col">
                        <span className="text-[0.58rem] text-slate-400">単価利益</span>
                        <span
                          className={`text-[0.72rem] font-black font-mono truncate ${
        ...
        <div className="flex flex-col h-full overflow-hidden px-2 pb-2 pt-1.5 gap-2">
          {!selectedItem ? (
            ...
            <>
              ...
              {/* 一括設定バー: 全HQ/全NQ, 全自給/調達 */}
              <div className="flex items-center justify-between px-2 py-1 bg-slate-900/90 rounded-xl border border-white/10 text-[0.65rem] shrink-0 gap-2 shadow-sm">
                <div className="flex items-center gap-1">
                  <span className="text-slate-400 font-bold flex items-center gap-0.5">
                    <i className="fa-solid fa-bolt text-amber-400 text-[0.55rem]"></i>品質:
                  </span>
                  <button
                    type="button"
                    onClick={handleSetAllHq}
                    className="px-1.5 py-0.5 rounded bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/40 font-bold transition-all text-[0.62rem]"
                    title="全素材をHQ相場で計算"
                  >
                    全HQ
                  </button>
                  <button
                    type="button"
                    onClick={handleSetAllNq}
                    className="px-1.5 py-0.5 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-600 font-bold transition-all text-[0.62rem]"
                    title="全素材をNQ相場に戻す"
                  >
                    全NQ
                  </button>
                ...
                <div className="flex items-center gap-1">
                  <span className="text-slate-400 font-bold flex items-center gap-0.5">
                    <i className="fa-solid fa-leaf text-emerald-400 text-[0.55rem]"></i>自給:
                  </span>
                  <button
                    type="button"
                    onClick={handleSetAllSelfSufficient}
                    className="px-1.5 py-0.5 rounded bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/40 font-bold transition-all text-[0.62rem]"
                  ...
                  <button
                    type="button"
                    onClick={handleClearSelfSufficient}
                    className="px-1.5 py-0.5 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-600 font-bold transition-all text-[0.62rem]"
                    title="自給を解除して調達に戻す"
                  >
                    調達
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-1.5 text-[0.58rem] mt-0.5 flex-wrap">
                                    {isShop ? (
                                    ...
                                    {mat.hasHqOption && (
                                      <button
                                        ...
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          handleToggleQuality(mat.itemId);
                                        }}
                                        className={`px-1 py-0.2 rounded border text-[0.55rem] font-bold transition-all ${
                                          mat.quality === 'hq'
                                            ? 'bg-amber-500/30 text-amber-300 border-amber-500/60'
                                            : 'bg-slate-800 hover:bg-slate-700 text-slate-400 border-slate-700'
                                        }`}
                                      >
                                        {mat.quality === 'hq' ? 'HQ' : 'NQ'}
                                      </button>
                                    )}
                                    {!isCrystalItem(mat.name) && mat.method !== 'craft' && (
                                      <button
                                        type="button"
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          handleToggleSelfSufficient(mat.itemId);
                                        }}
                                        className={`px-1 py-0.2 rounded border text-[0.55rem] font-bold transition-all ${
                                          mat.isSelfSufficient
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                              ...
                              ) : ing.isSelfSufficient ? (
                                <span className="px-1 py-0.2 rounded bg-emerald-950/70 text-emerald-300 border border-emerald-500/40 text-[0.55rem] font-bold">
                                  自給 (0G)
                                </span>
                              ) : (
                              ...
                              {!isCrystal && (
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleToggleSelfSufficient(ing.id);
                                  }}
                                  className={`px-1 py-0.2 rounded border text-[0.55rem] font-bold transition-all ${
                                    ing.isSelfSufficient
                                      ? 'bg-emerald-500/30 text-emerald-300 border-emerald-500/60'
