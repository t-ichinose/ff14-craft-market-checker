...
export const MobileArbitrageView: React.FC<MobileArbitrageViewProps> = React.memo(({
  ...
  // 仕入・販売画面内の表示モード ('history' | 'listings') - デフォルトは出品一覧
  const [sourceViewMode, setSourceViewMode] = useState<'listings' | 'history'>('listings');
  const [homeViewMode, setHomeViewMode] = useState<'listings' | 'history'>('listings');
  ...
  }, [allData, homeWorld, sortMode, minVelocity, excludeCrystals, searchQuery, availableCategories, selectedCategories, sourcingScope, itemsCatalog, listingsMap]);
    ...
    <div className="shrink-0 flex flex-col gap-1.5">
      ...
      {isMenuOpen && (
        <div className="flex flex-col gap-1.5 bg-slate-900/95 p-2 rounded-xl border border-white/10 shadow-xl transition-all">
          ...
          {/* Row 2: 仕入れDC範囲選択 */}
          <div className="flex items-center gap-1.5 bg-black/40 px-2 py-1.5 rounded-lg border border-white/10 text-xs">
            <span className="text-slate-400 font-bold text-[0.72rem] flex items-center gap-1 shrink-0">
              <i className="fa-solid fa-boxes-packing text-[#ffb703]"></i> 仕入れ:
            </span>
            <div className="flex gap-1 flex-1">
              {[
                { key: 'all_dc', label: '全DC (32鯖)' },
                { key: 'dc', label: '同DC (8鯖)' },
              ].map((scope) => (
                <button
                  key={scope.key}
                  onClick={() => setSourcingScope(scope.key as any)}
                  className={`flex-1 py-1 text-center rounded-md font-bold transition-all text-[0.7rem] ${
                    sourcingScope === scope.key
          ...
          {/* Row 4: ソートタブ（日当利益 / 単価利益 / 利益率 / 日販数） + クリスタル除外アイコン */}
          <div className="flex bg-black/40 p-0.5 rounded-lg border border-white/10 text-xs gap-1 items-center">
            ...
            <button
              ...
              className={`px-2 py-1.5 rounded-md text-xs font-bold transition-all flex items-center justify-center shrink-0 ${
                excludeCrystals
                    ...
                    />
                      ...
                      <div className="grid grid-cols-4 gap-1">
                        ...
                        <div className="bg-slate-950/70 border border-white/10 border-l-2 border-l-[#c084fc] rounded p-1 text-center">
                          <div className="text-[0.55rem] text-slate-400">目標仕入額 (上限)</div>
                  ...
                  loadingSource ? (
                    ...
                    <div className="flex flex-col gap-2">
                      {sourceListings.slice(0, 50).map((l, idx) => {
                        ...
                        // 割安 / 割高ガイド判定 (目標仕入上限額 maxBuyPrice との比較)
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between pt-0.5">
                              <div className="flex items-center gap-2 text-slate-400 text-[0.72rem] min-w-0">
                                {isLowest ? (
                                  <span className="bg-[#4ade80]/20 text-[#4ade80] border border-[#4ade80]/50 px-1.5 py-0.2 rounded text-[0.6rem] font-bold font-mono flex items-center gap-1 shadow-sm shrink-0">
                                    <i className="fa-solid fa-crown text-[0.55rem] text-[#ffb703]"></i> 最安 ({diffPctStr})
                                  </span>
                                ) : isGoodBuy ? (
                                  <span className="bg-[#38bdf8]/15 text-[#38bdf8] border border-[#38bdf8]/40 px-1.5 py-0.2 rounded text-[0.6rem] font-bold font-mono flex items-center gap-1 shadow-sm shrink-0">
                                    <i className="fa-solid fa-circle-check text-[0.55rem]"></i> 仕入可 ({diffPctStr})
                                  </span>
                                ) : (
                  ...
                  <div className="p-2 pt-0 flex flex-col gap-2">
                    ...
                    <div className="grid grid-cols-4 gap-1">
                      ...
                      <div className="bg-slate-950/70 border border-white/10 border-l-2 border-l-[#ffb703] rounded p-1 text-center">
                        ...
                        <div className="text-[0.72rem] font-bold font-mono text-[#ffb703] truncate">
                          {selectedOp.sellPrice > 0 ? `${selectedOp.sellPrice.toLocaleString()}G` : '-'}
                        </div>
                      </div>
                      <div className="bg-slate-950/70 border border-white/10 border-l-2 border-l-[#4ade80] rounded p-1 text-center">
